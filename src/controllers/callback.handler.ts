...
export const callbackQueryHandler = async (
  ...
  try {
    ...
    if (data.command === "add_monitor") {
      await addMonitorHandler(bot, callbackMessage);
      return;
    }

    const usemonitorStr = "usemonitor_";
    if (data.command.includes(usemonitorStr)) {
      const monitor_name = data.command.slice(usemonitorStr.length);
      await useMonitorHandler(bot, callbackMessage, monitor_name);
      return;
    }

    const delmonitorStr = "delmonitor_";
    if (data.command.includes(delmonitorStr)) {
